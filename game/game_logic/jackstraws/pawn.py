...
from game.game_logic.logic_func import is_in_chess_board, change_pos_name, is_checker_on_pos, is_pos_empty
...
class Pawn(Jackstraw):
    ...
    def _can_move_two_fields_forward(self, board: dict[str]) -> bool:
        return self.pos[1] == '2' and is_pos_empty(board[self.pos[0] + '3']) and is_pos_empty(board[self.pos[0] + '4'])
    ...
    def _add_field_forward_move(self, board: dict[str], moves: list[str]) -> None:
        ...
        if is_in_chess_board(field_forward) and is_pos_empty(board[field_forward]):
